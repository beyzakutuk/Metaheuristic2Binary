function [X_best,Best_fit,All_fit,nfes,res_det]= apgsk_imode(fhd, dimension, maxIteration, fNumber)
...
while stop_con==0 % nfes<max_nfes
    ...
    if (rand<=Probs(1))
        ...
        [EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, nfes,F,cr, res_det] = ...
            ...
            archive_freq, Par.xmin, Par.xmax,  Par.n,  PS1,  nfes, func_num,Par.Printing,Par.max_nfes, Par.Gmax,F,cr,fhd,res_det);
    ...
    if(rand<Probs(2))
        %% Apply APGSK
        [EA_2,EA_obj2,Par,nfes,res_det]= APGSK_fun(EA_2,EA_obj2,lu,func_num,Par,nfes,fhd,res_det);
        if(~isempty(Par.Record_FEs))
            if (nfes>=Par.Record_FEs(1))
...
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,F,cr,res_det ] = ...
    ...
    PopSize,  current_eval, I_fno,Printing,Max_FES, G_Max,F,cr,fhd,res_det)
...
function [pop,fitness,Par,nfes,res_det]= APGSK_fun(pop,fitness,lu,func,Par,nfes,fhd,res_det)
