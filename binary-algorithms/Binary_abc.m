function [bestSolution, bestFitness, iteration]=Binary_abc(fhd, dimension, maxIteration, fNumber, T_num)
...
BinaryFoods = zeros(FoodNumber, D);  % BURASI GÜNCELLENDİ
ObjVal = zeros(1, FoodNumber);

for i = 1:FoodNumber
    BinaryFoods(i,:) = discretizeSolution(Foods(i,:), T_num, []);
    %disp(['Binary çözüm: ', mat2str(BinaryFoods(i,:))]);
    ObjVal(i) = testFunction(BinaryFoods(i,:)', fhd, fNumber);
end

%ObjVal=testFunction(Foods', fhd, fNumber); 
...
%%%%%%%%% EMPLOYED BEE PHASE %%%%%%%%%%%%%%%%%%%%%%%%
    for i=1:(FoodNumber)
       ...
       %  /*if generated parameter value is out of boundaries, it is shifted onto the boundaries*/
        ...
        prevBinary = BinaryFoods(i,:); % BURASI GÜNCELLENDİ.
        newBinary = discretizeSolution(sol, T_num, prevBinary);

        %disp(['Binary çözüm: ', mat2str(newBinary)]);
        ObjValSol = testFunction(newBinary', fhd, fNumber);

        %ObjValSol=testFunction(sol', fhd, fNumber);
       ...
       if (FitnessSol>Fitness(i)) %/*If the mutant solution is better than the current solution i, replace the solution with the mutant and reset the trial counter of solution i*/
            Foods(i,:)=sol;
            BinaryFoods(i,:) = newBinary; % BURASI GÜNCELLENDİ.
...
if (trial(ind)>limit)
    ...
    prevBinary = BinaryFoods(ind, :);    % Eski çözümün binary hali
    newBinary = discretizeSolution(sol, T_num, prevBinary);
