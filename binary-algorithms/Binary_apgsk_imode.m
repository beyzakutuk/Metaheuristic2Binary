function [X_best,Best_fit,All_fit,nfes,res_det]= Binary_apgsk_imode(fhd, dimension, maxIteration, fNumber,T_num)
...
func_num=fNumber;
D=dimension;
Par=Introd_Par(func_num,D,max_nfes);
...
Fit=testFunction(BinaryPop', fhd, func_num);
%Fit=testFunction(Pop', fhd, func_num);
nfes = NP;
...
EA_1= Pop(1:PS1,:);    
EA_obj1= Fit(1:PS1);
Binary_EA_1 = BinaryPop(1:PS1,:);
%% APGSK
EA_2= Pop(PS1+1:size(Pop,1),:);    
EA_obj2= Fit(PS1+1:size(Pop,1));
Binary_EA_2 = BinaryPop(PS1+1:end,:);
...
while stop_con==0 % nfes<max_nfes
    ...
    if (rand<=Probs(1))
        %% apply IMODE % Binary_EA_1 içeri gönderdik ve dönüş değeri olarak aldık
        [EA_1, Binary_EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, nfes,F,cr, res_det] = ...
            IMODE( EA_1, Binary_EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,....
            archive_freq, Par.xmin, Par.xmax,  Par.n,  PS1,  nfes, func_num,Par.Printing,Par.max_nfes, Par.Gmax,F,cr,fhd,res_det,T_num);
        ...
        if PS1 > plan_NP1
            ...
            for r = 1 : reduction_ind_num
                ...
                EA_1(worst_ind,:) = [];
                EA_obj1(worst_ind,:) = [];
                Binary_EA_1(worst_ind,:) = [];  % BINARY popülasyondan da sil
            end
            archive.NP = PS1;
    ...
    if(rand<Probs(2))
        %% Apply APGSK
        [EA_2,Binary_EA_2, EA_obj2,Par,nfes,res_det]= APGSK_fun(EA_2,Binary_EA_2,EA_obj2,lu,func_num,Par,nfes,fhd,res_det,T_num);
        if(~isempty(Par.Record_FEs))
            if (nfes>=Par.Record_FEs(1))
        ...
        if PS2 > plan_NP2
            ...
            for r = 1 : reduction_ind_num
                ...
                EA_2(worst_ind,:) = [];
                EA_obj2(worst_ind,:) = [];
                Binary_EA_2(worst_ind,:) = [];  % BINARY popülasyondan da sil
                Par.K(worst_ind,:)=[];
            end
        end
    end
        ...
        Fit=[EA_obj1;EA_obj2];
        Pop=[EA_1;EA_2];
        Binary_Pop = [Binary_EA_1; Binary_EA_2];  % binary popülasyonları da birleştir
...
%         res_det= [res_det; repmat(Best_fit,PopSize,1)];
    ...
    disp(['fit çözüm: ', mat2str(Best_fit)]);
end
...
% =========================================================================
function [Par] = Introd_Par(I_fno,n,maxfes)
...
% karam_sallam@zu.edu.eg
% =========================================================================
function [x, Binary_EA_1,xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,F,cr,res_det ] = ...
    IMODE( x,Binary_EA_1,xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, xmin, xmax,  n,...
    PopSize,  current_eval, I_fno,Printing,Max_FES, G_Max,F,cr,fhd,res_det,T_num)
...
BinaryUi = zeros(size(ui));
for i = 1:PopSize
    BinaryUi(i,:) = discretizeSolution(ui(i,:), T_num, Binary_EA_1(i,:));  % prev olarak BinaryPop(i,:)
end
fitx_new = testFunction(BinaryUi', fhd, I_fno);
%fitx_new = testFunction(ui', fhd, I_fno);
...
function [pop, Binary_EA_2, fitness,Par,nfes,res_det]= APGSK_fun(pop,Binary_EA_2,fitness,lu,func,Par,nfes,fhd,res_det,T_num)
...
BinaryUi = zeros(size(ui));
for i = 1:pop_size
    BinaryUi(i,:) = discretizeSolution(ui(i,:), T_num, Binary_EA_2(i,:));  % prev olarak BinaryPop(i,:)
end
        
children_fitness = testFunction(BinaryUi', fhd, func);
...
Binary_EA_2(Child_is_better_index == 2, :) = BinaryUi(Child_is_better_index == 2, :);
