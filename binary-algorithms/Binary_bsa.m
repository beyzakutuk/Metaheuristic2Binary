function [bestSolution, bestFitness, iteration]=Binary_bsa(fhd, dimension, maxIteration, fNumber, T_num)
...
for i = 1:popsize
    fitnesspop(i) = testFunction(BinaryPop(i,:)', fhd, fNumber); 
end

%fitnesspop = testFunction(pop', fhd, fNumber);         % BURAYI GÜNCELLEDİK !!
...
fe=0;
    while fe<maxIteration
        ...
        % SELECTON-II
         ...
         % BINARY çözüm üret
        BinaryOffsprings = zeros(popsize, dim);
        for i = 1:popsize
            BinaryOffsprings(i,:) = discretizeSolution(offsprings(i,:), T_num, BinaryPop(i,:));  % prevSol olarak mevcut çözüm
        end
    
        % Evaluate fitness
        fitnessoffsprings = zeros(popsize, 1);
        for i = 1:popsize
            fitnessoffsprings(i) = testFunction(BinaryOffsprings(i,:)', fhd, fNumber); 
        end
        
        %fitnessoffsprings = testFunction(offsprings', fhd, fNumber);      % BURAYI GÜNCELLEDİK !!
        ...
        BinaryPop(ind,:) = BinaryOffsprings(ind,:); % BURAYI GÜNCELLEDİK !!
