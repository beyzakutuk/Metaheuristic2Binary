function [bestSolution, bestFitness, iteration]=Binary_cmaes(fhd, dimension, maxIteration, fNumber, T_num)
...
empty_individual.BinaryPosition = [];  % <<< EKLENDİ

M = repmat(empty_individual, MaxIt, 1);
M(1).Position = unifrnd(VarMin, VarMax, VarSize);
M(1).Step = zeros(VarSize);
M(1).BinaryPosition = discretizeSolution(M(1).Position, 8, []);  % ilk iterasyon, prev yok
M(1).Cost = testFunction(M(1).BinaryPosition', fhd, fNumber);
...
for g = 1:MaxIt
    ...
    for i = 1:lambda
        ...
        if g == 1
            prev = M(1).BinaryPosition;
        else
            prev = M(g).BinaryPosition;
        end
        pop(i).BinaryPosition = discretizeSolution(pop(i).Position, T_num, prev);
        pop(i).Cost = testFunction(pop(i).BinaryPosition', fhd, fNumber);
        % Evaluation
    ...
    M(g+1).BinaryPosition = discretizeSolution(M(g+1).Position, T_num, M(g).BinaryPosition);
    M(g+1).Cost = testFunction(M(g+1).BinaryPosition', fhd, fNumber);
