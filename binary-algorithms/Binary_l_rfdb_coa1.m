function [bestSolution, bestFitness, iteration]=Binary_l_rfdb_coa1(fhd, dimension, maxIteration, fNumber, T_num)
...
BinaryCoyotes = zeros(pop_total, D);
for c=1:pop_total
    BinaryCoyotes(c,:) = discretizeSolution(coyotes(c,:), T_num, []);
    costs(c,1) = testFunction(BinaryCoyotes(c,:)', fhd, fNumber); % BURAYI GÜNCELLEDİK!!
    disp(['Binary çözüm: ', mat2str(BinaryCoyotes(c,:))]);
...
while nfeval<nfevalMAX % Stopping criteria
    ...
    for p=1:n_packs
        ...
        for c=1:n_coy_aux
            ...
            binaryNew = discretizeSolution(new_coyotes(c,:), T_num, BinaryCoyotes(packs(p,c),:));
            new_cost = testFunction(binaryNew', fhd, fNumber); % BURAYI GÜNCELLEDİK!!
            disp(['Binary çözüm: ', mat2str(binaryNew)]);
            ...
            if new_cost < costs_aux(c,1)
                ...
                BinaryCoyotes(packs(p,c),:) = binaryNew;   % BURAYI GÜNCELLEDİK!!
            end
        end
        ...
        prevSol = BinaryCoyotes(packs(p, parents(1)), :);
        binaryPup = discretizeSolution(pup, T_num, prevSol);
        disp(['Binary çözüm: ', mat2str(binaryPup)]);
        pup_cost = testFunction(binaryPup', fhd, fNumber);  % BURAYI GÜNCELLEDİK!!
        ...
        if ~isempty(worst)
            ...
            BinaryCoyotes(packs(p,which(1)),:) = binaryPup;   % BURAYI GÜNCELLEDİK!!
        end
