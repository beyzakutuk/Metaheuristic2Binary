function [bestSolution, bestFitness, iteration]=Binary_lshade_epsin(fhd, dimension, maxIteration, fNumber, T_num)
...
if problem_size == 100
    ...
    % Örnek: pop matrisini binary'ye çevir
    ...
    BinaryPopLS = zeros(size(popLS));
    for i=1:size(popLS,1)
        BinaryPopLS(i,:) = discretizeSolution(popLS(i,:), T_num,  BinaryPop(i,:));
    end
    fitness_LS = testFunction(BinaryPopLS', fhd, func);
    ...
    while nfes < max_nfes
      ...
      for i = 1:size(ui,1)
          BinaryUi(i,:) = discretizeSolution(ui(i,:), T_num, BinaryPop(i,:));  % prev olarak BinaryPop(i,:)
      end
...
%      isempty(popold(I == 1, :))   
      ...
      popold = pop;
      popold(I == 2, :) = ui(I == 2, :);
      BinaryPop(I == 2, :) = BinaryUi(I == 2, :);  %  BURAYI GÜNCELLEDİK !!
      ...
      if flag_LS == true
          ...
          for gen_LS = 0 : GenMaxSelected
              ...
              for i = 1 : popsize_LS
                  [NP, fit,BinaryPop(i,:)] = LS_Process(popLS(i,:),S,gg,BestPoint,fhd, BinaryPop(i,:),T_num);
...
function [createPoint, fitness,PrevBinary] = LS_Process(Point,S,g,BestPoint,fhd, PrevBinary,T_num)
    ...
    GeneratePointDiscrete = discretizeSolution(GeneratePoint, T_num, PrevBinary);
    fitness = testFunction(GeneratePointDiscrete', fhd, S.FuncNo);

    PrevBinary = GeneratePointDiscrete;
