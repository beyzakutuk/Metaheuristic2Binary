function [bestSolution, bestFitness, iteration]=Binary_olshade(fhd, dimension, maxIteration, fNumber,T_num)
...
% Binary popülasyonu ilk kez sürekli pop'dan dönüştür
BinaryPop = zeros(pop_size, dimension);
for i = 1:pop_size
    % İlk nesilde prev yok => []
    BinaryPop(i,:) = discretizeSolution(pop(i,:), T_num, []);    % random
end
prevBinaryPop = BinaryPop;   % prevBinaryPop tanımlandı (önceki binary çözümler)

fitness = zeros(pop_size,1);
for i = 1:pop_size
    fitness(i) = testFunction(BinaryPop(i,:)', fhd, fNumber);
end
...
stop_con    = 0;
while stop_con == 0
    [pop,BinaryPop,fitness,pop_near_idx,archive_f,archive_Cr,hist_pos,bestold, bestx, current_eval,res_det] =...
        oDE(pop,BinaryPop,fitness,pop_near_idx,archive_f,archive_Cr,hist_pos,bestold, bestx, memory_size...
        , Par.xmin, Par.xmax, current_eval, Par.nich_size,res_det,Par.Printing,fhd, fNumber,T_num);
...
%                 Par.xmin, Par.xmax,  Par.n,  PS1,  current_eval, I_fno,res_det,Par.Printing);
            [EA, BinaryPop,EA_obj,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,current_eval,res_det] = ...
                LSHADE_MODE_greedy( EA,prevBinEA,EA_obj,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,....
                Par.xmin, Par.xmax,  Par.n,  PS1,  current_eval,res_det,Par.Printing,Par.Max_FES,fhd, fNumber,T_num);
...
function [pop,BinPop,fitness,pop_near_idx,archive_f,archive_Cr,hist_pos,bestold, bestx, current_eval, res_det] =...
        oDE(pop,BinPop,fitness,pop_near_idx,archive_f,archive_Cr,hist_pos,bestold, bestx, memory_size,....
        ~, ~, current_eval, nich_size, res_det, Printing,fhd, fNumber,T_num)   
...
%     %%Evaluate new member
        ...
        binX1 = discretizeSolution(X1, T_num, BinPop(jj,:));
        children_fitness(jj) = testFunction(binX1', fhd, fNumber);
...
function [x,BinaryPop,fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,current_eval,res_det] = ...
    LSHADE_MODE_greedy( x,BinaryPop,fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr, xmin, xmax,  n,...
    PopSize,  current_eval,res_det,Printing,Max_FES,fhd, fNumber,T_num)
...
% 3.1) Binary discretizasyon
    BinaryUi = zeros(PopSize, n);
    fitx_new = inf(PopSize,1);
    for i = 1:PopSize
        BinaryUi(i,:) = discretizeSolution(ui(i,:), T_num, BinaryPop(i,:));
        fitx_new(i) = testFunction(BinaryUi(i,:)', fhd, fNumber);
    end
