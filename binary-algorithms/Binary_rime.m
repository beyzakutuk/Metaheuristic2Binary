function [bestSolution, bestFitness, iteration]=Binary_rime(fhd, dimension, maxIteration, fNumber, T_num)
...
binaryRimepop = zeros(size(Rimepop));
for i = 1:N
    binaryRimepop(i,:) = discretizeSolution(Rimepop(i,:), T_num, []); % populasyon binary hale çevrildi
end

for i=1:N
    Rime_rates(1,i)=testFunction(binaryRimepop(i,:)', fhd, fNumber);
    %Rime_rates(1,i)=testFunction(Rimepop(i,:)', fhd, fNumber);     %  BURAYI GÜNCELLEDİK !!
...
while it <= Max_iter
    ...
    normalized_rime_rates = Rime_rates ./ sqrt(sum(Rime_rates.^2, 2)); % toolbox yüklü değilse !!
    ...
    for i=1:N
        ...
        binaryNewRimepop = zeros(size(newRimepop));
        for i = 1:N
            binaryNewRimepop(i,:) = discretizeSolution(newRimepop(i,:), T_num, binaryRimepop(i,:)); % binaryRimepop eski çözümü tutuyor
        end

        newRime_rates(1,i)=testFunction(binaryNewRimepop(i,:)', fhd, fNumber);
        %newRime_rates(1,i)=testFunction(newRimepop(i,:)', fhd, fNumber);       %  BURAYI GÜNCELLEDİK !!

        %Positive greedy selection mechanism
        if newRime_rates(1,i)<Rime_rates(1,i)
            Rime_rates(1,i) = newRime_rates(1,i);
            Rimepop(i,:) = newRimepop(i,:);
            binaryRimepop(i,:) = binaryNewRimepop(i,:); %burada yeni çözümle güncelliyoruz
