function [bestSolution, bestFitness, iteration]=Binary_sfs(fhd, dimension, maxIteration, fNumber, T_num)

mysettings;

S.Start_Point = 50;            
S.Maximum_Diffusion = 1;
S.Walk = 1; % *Important
S.Ndim = dimension;
S.Lband = lbArray;
S.Uband = ubArray;
S.Maximum_Generation  = ceil(maxIteration / ((S.Maximum_Diffusion+2)*S.Start_Point+S.Start_Point));
...
%Calculating the fitness of first created points=========================== 
    ...
    BinaryPoint = zeros(size(point));   % Binary çözüm matrisi

    for i = 1 : size(point,1)
        BinaryPoint(i,:) = discretizeSolution(point(i,:), T_num, []); % random
        FirstFit(i) = testFunction(BinaryPoint(i,:)', fhd, fNumber);
        %FirstFit(i) = testFunction(point(i,:)', fhd, fNumber);
    end
...
for G = 1  : S.Maximum_Generation
    ...
    for i = 1 : size(point,1)
        ...
        [NP, fit] = Diffusion_Process(point(i,:),S,G,BestPoint,fhd,fNumber,BinaryPoint(i,:),T_num);
        %[NP, fit] = Diffusion_Process(point(i,:),S,G,BestPoint,fhd,fNumber);,

        New_Point(i,:) = NP; 
        FitVector(i) = fit;

        BinaryPoint(i,:) = discretizeSolution(NP, T_num, prevBinary(i,:));
    end
    %======================================================================
    ...
    BinaryP = zeros(S.Start_Point, S.Ndim);     %yeni hesaplanan P binary formata çevrilir
    for i = 1:S.Start_Point
        BinaryP(i,:) = discretizeSolution(P(i,:), T_num, prevBinary(i,:));
    end

    Fit_FirstProcess = zeros(1,S.Start_Point);
    for i = 1 : S.Start_Point
        Fit_FirstProcess(i) = testFunction(BinaryP(i,:)', fhd, fNumber);
        %Fit_FirstProcess(i) = testFunction(P(i,:)', fhd, fNumber);
    end
    ...
    for i = 1 : S.Start_Point
       if rand > Pa(i)
           ...
           R2 = ceil(rand*size(point,1));
            ...
            BinaryReplace = discretizeSolution(ReplacePoint, T_num, prevBinary(i,:));
            
            if testFunction(BinaryReplace', fhd, fNumber) < testFunction(BinaryPoint(i,:)', fhd, fNumber)
                point(i,:) = ReplacePoint;
                BinaryPoint(i,:) = BinaryReplace;  % binary çözüm de güncellenmeli
            end
            %if testFunction(ReplacePoint', fhd, fNumber) < testFunction(point(i,:)', fhd, fNumber)
            %    point(i,:) = ReplacePoint;
            %end
...
function [createPoint, fitness] = Diffusion_Process(Point,S,g,BestPoint,fhd,fNumber, BinaryPrev,T_num)
    ...
    for i = 1 : size(New_Point,1)
        BinarySol = discretizeSolution(New_Point(i,:), T_num, BinaryPrev);
        fitness(i) = testFunction(BinarySol', fhd, fNumber);
        %fitness(i) = testFunction(New_Point(i,:)', fhd, fNumber);
    end
